...
def almost_eq_tri_naive(a):
    ...
    """
    perimeter = []
    area = [ (b_ * np.sqrt( decimal.Decimal(pow(a,2)) - \
...
def almost_eq_tri(a):
    ...
    sum_=0
    for b in blist:
        two_b_m = np.sqrt(a**2 - (b/2)**2)
        if np.mod(two_b_m,1)==0 and (two_b_m % 2 == 0 or b % 2 == 0): 
            sum_ += a * 2 + b
    if sum_ or a == 302828:
        print(f'{a}:\t [{(np.sqrt(a**2 - (blist[0]/2)**2)*blist[0]/2)}, {(np.sqrt(a**2 - (blist[1]/2)**2)*blist[1]/2)}] {sum_}')
    return sum_
...
def calc94():
    limit = 1000000000
    sum_ = 0
    for a in range(2, int(limit / 3)):
        sum_ += almost_eq_tri_naive(a)
    return sum_

print(timer_wrapper(calc94))
# performance_test()
