...
def load_triangles(triangle_file: str) -> Triangle:
    triangles = []
...
def points_dist(point1: Point, point2: Point) -> float:
    X1, Y1 = point1
    X2, Y2 = point2
    return sqrt((X1 - X2)**2 + (Y1 - Y2)**2)


def angle_by_law_of_cosines(point1: Point, vertex: Point, point2: Point) -> float:
    X1, Y1 = point1
    XV, YV = vertex
    X2, Y2 = point2
...
def zero_out_or_in(triangle: Triangle) -> bool:
...
def calc102(triangles: Triangle) -> int:
    return len([1 for t in triangles if zero_out_or_in(t)])
...
if __name__ == '__main__':
    ...
    if testing:
        ...
        result = timer_wrapper(calc102, [triangles])
        print(f"{result = }")
